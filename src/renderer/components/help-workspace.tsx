...
const HELP_ENTRIES: ReadonlyArray<{ description: string; label: string; route: PivotRoute }> = [
  ...
  { description: 'Ask questions and share tips', label: 'Community Forum', route: 'docs' },
]
...
export function HelpWorkspace({ onNavigate }: { onNavigate: (route: PivotRoute) => void }): ReactElement {
  ...
  return <section className="pv-help-workspace" data-figma-screen="818:12562">
    ...
    <div className="pv-help-columns"><section><h2>FREQUENTLY ASKED QUESTIONS</h2>{entries.slice(0, 5).map((entry) => <HelpRow entry={entry} icon={Settings} key={entry.label} onNavigate={onNavigate} />)}</section><section><h2>RESOURCES</h2>{entries.slice(3).map((entry, index) => <HelpRow entry={entry} icon={[BookOpen, Keyboard, Code2, BookOpen][index] ?? BookOpen} key={entry.label} onNavigate={onNavigate} />)}</section></div>
  </section>
}
